...
class DeepQNetwork:
    ...
    def _create_network(self):
        ...
        # input layer
        # 当前状态
        self.state_input = tf.compat.v1.placeholder("float", [None, 80, 80, 4])
        ...
        # 当前状态选择的action
        self.action_input = tf.compat.v1.placeholder("float", [None, self.n_actions])
        # 当前状态实际的q_value
        self.act_q_value = tf.compat.v1.placeholder("float", [None])
        # 按照经验预测出的q_value(Q(s_t, a_t))
        pred_q_value = tf.reduce_sum(input_tensor=tf.multiply(self.q_value, self.action_input),
                                     axis=1)
        self.cost = tf.reduce_mean(input_tensor=tf.square(self.act_q_value - pred_q_value))
